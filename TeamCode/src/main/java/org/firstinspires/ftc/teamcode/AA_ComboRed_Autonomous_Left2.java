...
@com.qualcomm.robotcore.eventloop.opmode.Autonomous(name="AA ComboRed_Autonomous_Left")
public class AA_ComboRed_Autonomous_Left2 extends LinearOpMode {   //Linear op mode is being used so the program does not get stuck in loop()
...
//---------------- gyro/sensor way points -----------------------------------
            Sensors.setLED(BLUE_LED, true);

            //  ....asuming red closest table

            // drop color sensor
            // rotate and bop the correct jewel
            // rotate to original position
            // Jewel(2.0);


            // deploy the trolley track
            // raise the color sensor

            // move forward 4 inches
           // straightAbsolute(0, 2, .75);

            // rotate so camera can see pictograph


            // vufora the pictograph, returns 1,2, or 3
            // rotate so photoCell can see the tape

            // strafe and count the dividers until in position
            strafeLeftAbsolute(0, 5, .5);
            // go forward then stop
            // insert the glyph
            //
            turnAbsolute(90);
            telemetry.addData("I see ", eyeScan());
            telemetry.update();
...
//------------------------------------------------------------------------------------------------
    ...
    public void straightAbsolute(int target, long driveTime, double straightSpeed) {
        ...
        while (opModeIsActive() && (runtime.seconds() < driveTime)) {
           ...
           while (Math.abs(zAccumulated - target) > 3 && opModeIsActive()) {
               zAccumulated = mrGyro.getIntegratedZValue();  //Set variables to gyro readings
               telemetry.addData("gyro says =", zAccumulated);
               telemetry.update();
               if (zAccumulated < target) {  //if gyro is positive, we will turn right
    ...
    public void strafeLeftAbsolute(int target, long driveTime, double strafeSpeed) {
        telemetry.addData("Target = ", target);
        double knudge = .1;              //coarse correction value
        Sensors.setLED(RED_LED, true);
        FleftDrive.setPower(strafeSpeed);
        BleftDrive.setPower(-strafeSpeed);
        FrightDrive.setPower(-strafeSpeed);
        BrightDrive.setPower(strafeSpeed);
        ...
        while (opModeIsActive() && (runtime.seconds() < driveTime)) {
            ...
            while (Math.abs(zAccumulated - target) > 3 && opModeIsActive()) {
                ...
                Sensors.setLED(RED_LED,  true);
                if (zAccumulated < target) {  //if gyro is positive, we will turn right
                    FleftDrive.setPower(-(strafeSpeed + knudge));
                    BleftDrive.setPower(strafeSpeed + knudge);
                    FrightDrive.setPower(-(strafeSpeed + knudge));
                    BrightDrive.setPower(strafeSpeed + knudge);
                    Sensors.setLED(BLUE_LED, true);
                    Sensors.setLED(RED_LED,  false);
                }
                if (zAccumulated > target) {  //if gyro is positive, we will turn left
                    FleftDrive.setPower(strafeSpeed + knudge);
                    BleftDrive.setPower(-(strafeSpeed + knudge));
   ...
   /* public void PictoGraph(double holdTime) {
    ...
    public boolean eyeScan() {
        ...
        //telemetry.addData("I see in method ", eye);
        //telemetry.update();
        return eye;
    }
}
